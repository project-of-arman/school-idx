...
const resultFormSchema = z.object({
  student_id: z.coerce.number({
      required_error: "শিক্ষার্থী নির্বাচন আবশ্যক",
      invalid_type_error: "শিক্ষার্থী নির্বাচন আবশ্যক",
    }).min(1, "শিক্ষার্থী নির্বাচন আবশ্যক"),
...
export function ResultForm({ result, students }: { result?: ResultWithSubjects, students: StudentForResultForm[] }) {
  ...
  const { 
    ...
    defaultValues: result ? {
        ...
        subjects: result.subjects.map(s => ({...s, marks: s.marks ?? null}))
    } : {
      exam_name: "",
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div>
            ...
            />
            {errors.student_id && <p className="text-sm font-medium text-destructive mt-1">{errors.student_id.message}</p>}
          </div>
      ...
      <Card>
          ...
          <CardContent>
              ...
              <Button type="button" variant="outline" size="sm" onClick={() => append({ id: undefined, subject_name: '', marks: null, grade: '', gpa: '' })} className="mt-4"><PlusCircle className="mr-2 h-4 w-4" /> বিষয় যোগ করুন</Button>
          </CardContent>
      </Card>
