...
import { ResultForAdmin, deleteResult } from "@/lib/actions/results-actions";
...
export default function ResultsTable({ results }: { results: ResultForAdmin[] }) {
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [selectedResult, setSelectedResult] = useState<ResultForAdmin | null>(null);
  ...
  const handleDeleteClick = (result: ResultForAdmin) => {
  ...
  const handleConfirmDelete = async () => {
    if (selectedResult) {
      const result = await deleteResult(selectedResult.id);
      if (result.success) {
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                       <DropdownMenuItem asChild>
                        <Link href={`/results?roll=${result.roll}&class=${encodeURIComponent(result.class_name)}`} target="_blank">
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>এই ফলাফল স্থায়ীভাবে মুছে ফেলা হবে। এই কাজটি আর ফেরানো যাবে না।</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
